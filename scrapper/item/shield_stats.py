...
class ShieldStatsScrapper(CsvScrapper):
    """
    Weapon list scrapper.
    """

    def __init__(self, root_url):
        super(ShieldStatsScrapper, self).__init__(root_url + '/wiki/Shields', 'output/shield_stats.csv',
    ...
    def _extract_links(self, dom):
        main_list = dom.select('h2:has(> span#List_of_Shields) + table li a')
        main_list = main_list + dom.select('h2:has(> span#List_of_Shields) + table + table li a')
